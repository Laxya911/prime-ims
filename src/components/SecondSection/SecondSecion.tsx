...
import { generatePrintableContent } from "@/app/purchase/view/PrintableContent";
...
const SecondSection = () => {
  ...
  useEffect(() => {
    ...
    const filteredConsignments = filteredProductsData.filter((purchases) => {
      const lastStatus = purchases.status;
      if (lastStatus !== "Received") {
        // No filtering needed for superadmin
        return lastStatus;
      }
    });
    const sortedPurchaseOrder = filteredConsignments.sort(
  ...
  const productLength = productsOnPage.length > 0;
  const purchaseLength = purchaseOnPage.length > 0;
  if (loading) {
    return (
      <>
        <Loading />
        ...
        <div className="relative mb-4  sm:rounded-lg">
          <div className="flex flex-col sm:flex-row px-8 py-4 rounded bg-gray-600 justify-center sm:justify-between mt-6">
            ...
            <div className="text-lg text-center  mb-2">
              <p>
                Hi {session?.data?.user.name} you have{" "}
                <strong>{filteredProducts.length} </strong> Purchases Pending
              </p>
            </div>
                ...
                <table className=" w-full text-xs text-left text-gray-700 ">
                  ...
                  <tbody>
                    {productsOnPage.map((product, _id) => (
                      ...
                      >
                        <td className="px-1 py-2 border-b dark:border-[#eee] border-strokedark ">
                          {product.vName}
                        </td>
