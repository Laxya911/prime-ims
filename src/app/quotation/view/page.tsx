...
import Quotations from "@/app/commonApi/quotationApi";
...
const QuotationList = () => {
  ...
  const { allQuotation, handleDelete } = Quotations();
  ...
  useEffect(() => {
    ...
    const filteredProductsData = allQuotation
    ...
    const filteredPurchasesData = allQuotation.filter((purchase) => {
  ...
  const exportToExcel = () => {
    ...
    saveAs(excelBlob, "Quotations.xlsx");
  };
  ...
  const handlePrint = async (_id: string) => {
    try {
      const response = await axios.get(`/api/quotation/${_id}`);
  ...
  const handleUpdate = async (_id: string | undefined) => {
    try {
      ...
      router.push(`/quotation/view/${_id}`, { state: { invoiceData } } as any);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const dataLength = allQuotation.length > 0;
  if (session) {
    return (
      <>
                            ...
                            purchaseData?._id === product._id ? (
                              ...
                              >
                                <PDFDownloadLink
                                  ...
                                  fileName={`${product.customerName}_${product.qNumber}.pdf`}
                                >
