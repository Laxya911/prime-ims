...
const QuotationList = () => {
  ...
  const { allInvoices, handleDelete } = PrimeInvoiceApi();
  ...
  useEffect(() => {
    ...
    const filteredProductsData = allInvoices
    ...
    const filteredPurchasesData = allInvoices.filter((purchase) => {
  ...
  const handlePrint = async (_id: string) => {
    try {
      const response = await axios.get(`/api/primeinvoice/${_id}`);
  ...
  const handleUpdate = async (_id: string | undefined) => {
    try {
      ...
      router.push(`/invoice/view/${_id}`, { state: { invoiceData } } as any);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const dataLength = allInvoices.length > 0;
                            ...
                            purchaseData?._id === product._id ? (
                              ...
                              >
                                <PDFDownloadLink
                                  ...
                                  fileName={`${product.customerName}_${product.invoiceNumber}.pdf`}
                                >
